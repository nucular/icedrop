...
app.init = function() {
  ...
  $("#screen").bind("mousemove", function(e) {
    ...
    if (app.mousemoved) app.mousemoved(e.clientX, e.clientY, dx, dy);
...
app.frame = function() {
  ...
  if (elapsed > app.frameinterval) {
    ...
    app.draw(app.ctx, app.canvas.width, app.canvas.height, Math.min(app.canvas.width, app.canvas.height));
  }
}
